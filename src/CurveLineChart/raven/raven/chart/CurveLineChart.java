package CurveLineChart.raven.raven.chart;

import CurveLineChart.raven.chart.ModelChart;
import CurveLineChart.raven.chart.ModelLegend;
import DLL.ModelDataCurveLineChart;
...
public class CurveLineChart extends JComponent {
    ...
    public void setLineChartData(String[] month,int [] doanhthu,int [] loinhuan,int [] sodon){
        ArrayList<ModelDataCurveLineChart> data = new ArrayList<>();
        for(int i=0;i<month.length;i++)
            data.add(new ModelDataCurveLineChart(month[i], doanhthu[i], loinhuan[i],sodon[i]));

        //import data into chart
        for (int i = data.size() - 1; i >= 0; i--) {
            ModelDataCurveLineChart d = data.get(i);
            this.addData(new ModelChart(d.getMonth(), new double[]{d.getDoanhthu(), d.getLoinhuan(),d.getSodon()}));
        }
        this.start();
    }
